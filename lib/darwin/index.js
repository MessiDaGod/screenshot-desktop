...
function darwinSnapshot(options = {}) {
  const performScreenCapture = (displays) =>
    new Promise((resolve, reject) => {
      ...
      if (!Number.isInteger(displayId) || displayId < 0 || displayId > maxDisplayId) {
        const validChoiceMsg =
          maxDisplayId === 0
            ? "(valid choice is 0 or drop screen option altogether)"
            : `(valid choice is an integer between 0 and ${maxDisplayId})`;
      ...
      const coordinates = options.coordinates
        ? `-R ${options.coordinates.x},${options.coordinates.y},${options.coordinates.width},${options.coordinates.height}`
        : "";

      exec(
        `screencapture ${coordinates} -x -t ${format} ${pathsToUse.join(" ")}`,
        function (err, stdOut) {
          if (err) {
            return reject(err);
